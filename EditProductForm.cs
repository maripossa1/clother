...
namespace ClothingStoreApp
{
    public class EditProductForm : Form
    {
        ...
        private void InitializeForm()
        {
            ...
            this.Text = "✏️ MODIFIER UN PRODUIT - CAISSE";
            ...
            // Bouton sauvegarde
            saveBtn = new CustomButton("💾 MODIFIER LE PRODUIT", 270, 0, 350, 70, Color.FromArgb(16, 185, 129));
            saveBtn.Click += (s, e) => ModifierProduit();
        ...
        private void LoadProductData()
        {
            try
            {
                ...
                string query = @"SELECT name, price_without_tva, price, stock_quantity, size, color, category, barcode, tva_rate
                ...
                if (reader.Read())
                {
                    ...
                    // تحميل السعر (يفضل price_without_tva إذا كان موجودًا)
                    if (reader["price_without_tva"] != DBNull.Value && decimal.TryParse(reader["price_without_tva"].ToString(), out decimal prixHT))
                    {
                        priceBox.Value = prixHT;
                    }
                    else if (reader["price"] != DBNull.Value && decimal.TryParse(reader["price"].ToString(), out decimal prix))
                    {
                        priceBox.Value = prix;
                    }

                    if (reader["stock_quantity"] != DBNull.Value && int.TryParse(reader["stock_quantity"].ToString(), out int quantity))
                    {
                        quantityBox.Value = quantity;
                    }

                    categoryBox.Text = reader["category"]?.ToString() ?? "";
                    barcodeBox.Text = reader["barcode"]?.ToString() ?? "";

                    // تحميل إعدادات TVA
                    if (reader["tva_rate"] != DBNull.Value && decimal.TryParse(reader["tva_rate"].ToString(), out decimal tvaRate) && tvaRate > 0)
                    {
                        applyTvaCheckBox.Checked = true;
                        tvaRateBox.Value = tvaRate;
                    }

                    // تحميل الأحجام والألوان إذا كانت موجودة
                    ...
                    {
                        useSizesColorsCheckBox.Checked = true;
                        LoadSizesAndColors(sizes, colors);
                    }

                    // حساب TVA بعد تحميل البيانات
                    CalculerTVA();
                }
            }
            catch (Exception ex)
        ...
        private void LoadSizesAndColors(string sizes, string colors)
        {
            // تحميل الأحجام المختارة
            if (!string.IsNullOrEmpty(sizes))
            {
                string[] selectedSizes = sizes.Split(',');
                foreach (Control control in sizesFlowPanel.Controls)
                {
                    if (control is CheckBox checkBox && Array.Exists(selectedSizes, size => size.Trim() == checkBox.Text))
                    {
            ...
            if (!string.IsNullOrEmpty(colors))
            {
                ...
                {
                    if (control is CheckBox checkBox && Array.Exists(selectedColors, color => color.Trim() == checkBox.Text))
                    {
                        checkBox.Checked = true;
                    }
                }
            ...
            try
            {
                ...
                string query = @"UPDATE products 
                        SET name = @name, 
                            ...
                            barcode = @barcode, 
                            tva_rate = @tva_rate, 
                            price_without_tva = @price_without_tva,
                ...
                if (rowsAffected > 0)
                {
                    string messageSucces = "✅ PRODUIT MODIFIÉ AVEC SUCCÈS!\n\n" +
                                          $"📦 NOM: {nameBox.Text}\n" +
                                          $"💰 PRIX HT: {prixHT:N2} €\n" +
                                          $"🛒 PRIX TTC: {prixTTC:N2} €\n" +
                ...
                {
                    MessageBox.Show("Aucun produit trouvé avec cet ID.", "Erreur",
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
